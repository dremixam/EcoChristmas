...
- **Inflatable Santa:** Towering yard centerpiece with custom housing value.
- **Christmas Trees:** Craft small through huge trees, each tuned for room categories and diminishing returns so settlements can mix sizes without waste.
- **Gift Wraps:** Usable presents usable as single item storage, perfect for hiding real gifts while boosting home scores.
- **Snowman Companion:** A jolly, snow-proof friend crafted from everyday tailoring and farming materials.

## Getting Started

1. Drop the `EcoChristmasMod` folder into your `Mods/UserCode/` directory (or link it, as in this repo).
